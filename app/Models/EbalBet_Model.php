...
class EbalBet_Model extends Model 
{
    ...
    function getBetAccount($reqData){
        ...
        $strSql .= " SUM(CASE WHEN bet_type=0 THEN (bet_player + bet_banker - ABS(bet_player - bet_banker))/2 ELSE 0 END ) AS bet_bal_sum, ";
        $strSql .= " SUM(CASE WHEN bet_type=0 AND bet_result = 'Banker' THEN (bet_player + bet_banker - ABS(bet_player - bet_banker))/2 ELSE 0 END ) AS bet_bal_banker, ";
