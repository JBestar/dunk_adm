...
function ShowBetHistory(jsonBetData) {
    ...
    for (nRow in jsonBetData) {
        ...
        if (jsonBetData[nRow].bet_mode == 1 || jsonBetData[nRow].bet_mode == 2) {
            ...
            strBetMode = "<td class = 'pb-home-table-betmode-normal'> 슈퍼볼";
            ...
            strBetMode = "<td class = 'pb-home-table-betmode-normal'> 슈퍼볼조합";
        } else if (jsonBetData[nRow].bet_mode >= 13 && jsonBetData[nRow].bet_mode <= 20) {
            strBetMode = "<td class = 'pb-home-table-betmode-line'> 슈퍼+파워조합";
        } else if (jsonBetData[nRow].bet_mode == 21 || jsonBetData[nRow].bet_mode == 22) {
            strBetMode = "<td class = 'pb-home-table-betmode-line'> 숫자합";
        } else if (jsonBetData[nRow].bet_mode >= 23 && jsonBetData[nRow].bet_mode <= 26) {
            strBetMode = "<td class = 'pb-home-table-betmode-line'> 숫자합조합";
        } else if (jsonBetData[nRow].bet_mode >= 31 && jsonBetData[nRow].bet_mode <= 38) {
            strBetMode = "<td class = 'pb-home-table-betmode-line'> 슈퍼조합+파워홀짝";
        ...
        if (jsonBetData[nRow].bet_mode >= 1 && jsonBetData[nRow].bet_mode <= 4) {
            ...
            strBetTarget = getHtmlByBet(1, jsonBetData[nRow].bet_target);
            strResultTarget = getHtmlByBet(1, jsonBetData[nRow].bet_result);
        } else if(jsonBetData[nRow].bet_mode == 22){
            strBetTarget = getHtmlByBet(2, jsonBetData[nRow].bet_target);
            strResultTarget = getHtmlByBet(2, jsonBetData[nRow].bet_result);
        } else if (jsonBetData[nRow].bet_mode >= 23 && jsonBetData[nRow].bet_mode <= 26) {
            if (jsonBetData[nRow].bet_result.length > 1) {
                strResultTarget = getHtmlByBet("1", jsonBetData[nRow].bet_result.charAt(0)) + getHtmlByBet("2", jsonBetData[nRow].bet_result.charAt(1));
            }
            if (jsonBetData[nRow].bet_mode == 23) {
                strBetTarget = getHtmlByBet(1, "P") + getHtmlByBet(2, "P");
